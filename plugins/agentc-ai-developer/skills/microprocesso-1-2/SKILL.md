...
2. **Activate venv** - Load the isolated environment
3. **Install dependencies** - Core packages: langchain, anthropic, langsmith, python-dotenv
4. **Create .env** - Store API keys (LANGSMITH_API_KEY, ANTHROPIC_API_KEY)
5. **Create .env.example** - Template for team (no secrets)
6. **Create requirements.txt** - Lock dependency versions
7. **Create .gitignore** - Protect secrets from git commits
